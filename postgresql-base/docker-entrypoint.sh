...
if [ "$1" = 'postgres' ]; then
    ...
    if [ -z "$(ls -A "$PGDATA")" ]; then
        ...
        if [ "$POSTGRES_SUPER_PASSWORD" ]; then
            pass="PASSWORD '$POSTGRES_SUPER_PASSWORD'"
            authMethod=md5
        else
            ...
            cat >&2 <<-'EOWARN'
                ****************************************************
                WARNING: No password has been set for the database.
                         ...
                         system.
                         Use "-e POSTGRES_SUPER_PASSWORD=password" to set
                         it in "docker run".
                ****************************************************
EOWARN
        ...
        # assign FOO=bar if FOO is unset
        : ${POSTGRES_SUPER_USER:=postgres}
        # : ${POSTGRES_DB:=$POSTGRES_SUPER_USER}
...
# EOSQL
        ...
        if [ "$POSTGRES_SUPER_USER" = 'postgres' ]; then
            op='ALTER'
        else
            op='CREATE'
        fi
        ...
        # -E: Echo all commands.
        gosu postgres postgres --single -jE <<-EOSQL
            $op USER "$POSTGRES_SUPER_USER" WITH SUPERUSER $pass ;
EOSQL
        echo
